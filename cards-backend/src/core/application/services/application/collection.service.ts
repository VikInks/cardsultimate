...
export class CollectionService implements CollectionServiceInterface {
	...
	async publicCanView(ownerId: string, id: string, isPrivate: boolean): Promise<void> {
        try {
			if (isPrivate) {
				const collection = await this.collectionRepository.getById(id);
				if (collection?.idOwner === ownerId) {
					await this.collectionRepository.setViewCollection(id, isPrivate);
				}
			}
		} catch (e) {
			throw new CustomResponse(500, 'Error getting collection of user ${idOwner}, please contact support');
		}
    }
	...
	async create(ownerId: string): Promise<CollectionEntityInterface> {
		try {
			...
			const collection: CollectionEntityInterface = {id: id, idOwner: ownerId, cards: cards, updateDate: updateDate, isPrivate: true};
			return await this.collectionRepository.create(collection);
		} catch (e) {
			throw new CustomResponse(500, 'Error creating collection of user, please contact support');
		}
	}
	...
	async delete(id: string, ownerId: string): Promise<boolean> {
		try {
			...
			throw new CustomResponse(500, 'Error deleting collection of user, please contact support');
		}
	}
	...
	async update(item: CardsEntityInterface[], collectionId: string, ownerId: string): Promise<CollectionEntityInterface> {
		try {
			...
			throw new CustomResponse(500, 'Error updating collection of user, please contact support');
		}
	}
}